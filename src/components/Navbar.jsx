  ...
  return (
    <div className="absolute top-0 left-0 w-full z-10">
      ...
      <div className='container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32 bg-transparent'>
        ...
        <button onClick={() => setShowForm(true)} className='hidden md:block bg-white px-8 py-2 rounded-full'>
          Sign up
        </button>
        <img onClick={() => setShowMenu(true)} src={assets.menu_icon} className='md:hidden w-7 cursor-pointer' alt="Menu" />
      </div>
      ...
      <div
        ref={formRef}
        className={`${
          ShowForm ? 'fixed block' : 'hidden'
        } fixed top-20 right-10 h-fit w-full max-w-sm z-50 bg-gray-800 p-6 text-white shadow-lg rounded-lg sm:hidden md:block`}
      >
      ...
      <div className={`md:hidden ${ShowMenu ? 'fixed w-full' : 'h-0 w-0'} right-0 top-0 bottom-0 overflow-hidden bg-white transition-all duration-200`}>
        <div className='flex justify-end p-6 cursor-pointer'>
          <img onClick={() => setShowMenu(false)} src={assets.cross_icon} className='w-6' alt="Close" />
        </div>
